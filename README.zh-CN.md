...
- 个体基线标准化、加权多模态融合和一维 Kalman 平滑。
- ISO（先匹配后下降）与 DIRECT（直接引导）治疗目标轨迹。
- 带死区、积分泄漏、抗积分饱和和输出限幅的 PI 控制器。
...
误差位于死区时输出为零并泄放积分；信号置信度不足时退出实时闭环并取消尚未播放的命令。`MusicGrammar` 把控制量映射为速度、配器层级和派生音乐参数，并等待音频引擎明确报告的小节或乐句边界后才提交。
